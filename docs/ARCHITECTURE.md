...
- **角色权限保存与回显**：`PUT /api/roles/:id/permissions` 是全量替换语义，优先接收 `permissionCodes`，严格校验所有 code/id 必须存在；事务内先删除该角色所有 `RolePermission`，再按本次提交的权限集合重建，保存成功返回数据库最新 `permissionIds` / `permissionCodes` / `permissions`。后端不基于旧权限自动补子权限，也不会把公司管理相关权限自动补给运营专员；`GET /api/roles/:id` 与 `/api/auth/me` 均实时读取数据库中的 `RolePermission`。
...
| 方法 | 路径 | 权限 | 说明 |
|------|------|------|------|
| GET | `/api/store-products/grab-cart/readiness?shopId=&includePreview=` | `ACTION_STORE_PRODUCT_GRAB_CART` | 候选准备漏斗，默认 DB-only；`includePreview=true` 时做实时 preview 诊断 |
...
- `displaySummary`：`resellCount/inStockResellCount/dataReadyCount/candidateReadyCount`，给前端空状态直接展示。
- `blockers/topBlockers`：按 `count desc` 输出 `NOT_RESELL/OUT_OF_STOCK/ALREADY_WON/MISSING_PRODUCT_MAPPING/MISSING_COST/MISSING_FBE_FEE/MISSING_LOGISTICS/MISSING_COMMISSION/CART_PRICE_TAX_MODE_UNKNOWN/OFFER_NOT_SELLABLE/BELOW_FINAL_MIN_PRICE/OTHER`。
- `nextActions`：结构化运营动作 `{ action, description, priority }[]`；`legacyNextActions` 保留字符串说明，便于旧展示兼容。
...
**权限与统计口径**：所有接口必须登录；`dashboard`、`monthly-overview` 是公开看板，所有已登录用户均可查看，但登记对象和团队统计对象只包含**运营专员、运营主管**（`DAILY_REPORT_ROLE_NAMES`）。提交每日登记（`POST /reports`、`PUT /reports/:reportId`、旧 `POST /logs`）仅允许**运营专员、运营主管**（或未来 `ACTION_OPERATION_DAILY_SUBMIT`）；仓库专员/仓库主管及其他非运营角色提交返回 403。查看任意员工某日登记详情（`GET /users/:userId/report`、`GET /users/:userId/logs`）需拥有 `MENU_DASHBOARD_DAILY`（严格超级管理员自动通过），不再限制本人/主管/超管；无每日登记页面权限返回 403：`无权限查看员工运营日报`。运营专员/主管只能提交、修改自己的日报，提交后仅允许修改一次，`edit_count >= 1` 时再次修改返回 400。管理员/老板可看 dashboard、monthly-overview，但 Phase 1 不开放管理员修改别人日报，避免审计风险。新版月度总览只统计新 5 类：`PRODUCT_SELECTION` 选品数量、`PRODUCT_LISTING` 上新数量、`APPROVED_COUNT` 合规数量、`SHIPMENT_COUNT` 发货数量、`OTHER` 其他说明；旧 `QUALIFICATION/ADJUSTMENT` 不自动映射为合规/发货，避免历史口径失真。昨日未登记名单、本月积分榜 Top 5、热力图员工行、dashboard 人员排名和趋势累计均基于运营专员+运营主管 userId 集合过滤，仓库角色不参与每日登记统计。积分预留使用临时规则 `PRODUCT_SELECTION=1`、`PRODUCT_LISTING=2`、`APPROVED_COUNT=2`、`SHIPMENT_COUNT=1`、`OTHER=0`。热力图按月份 `workDate` 范围查询 reports/logs 并补齐整月日期，未来日期标记 `isFuture=true`，OTHER 仅返回短摘要，完整内容由 report 详情接口查看。
...
**权限与统计口径**：所有接口必须登录。创建任务时 `creatorId` 只能取 `req.user.userId`，禁止读取前端传入的 creator；`assigneeId` 必须是 ACTIVE 用户。assignee 只能把自己收到的任务改为 `TODO/IN_PROGRESS/DONE`，creator 只能取消自己创建的任务，不能替 assignee 标记 DONE；`DONE/CANCELLED` 后第一期禁止再改状态。`my-dashboard` 返回 `weekStart/weekEnd/lastWeekStart/lastWeekEnd`（可选 `weekStart` 参数指定当前周）；`summaryCards` 保留 `weeklyPendingCount/weeklyDoneCount/monthlyCompletionRate/receivedTaskCount`，并新增 `todoTaskCount/overdueTodoTaskCount/todayDueTaskCount`。`todoTasks`（兼容字段 `weeklyTasks` 同值）返回 `assigneeId=当前用户` 且非 `DONE/CANCELLED` 的全部待办，按逾期优先 → 截止日从近到远 → 同日内优先级高→中→低 → `updatedAt desc` 排序，不再限制本周截止。本周统计卡片仍按 `dueDate/completedAt` 落在所选周计算。`historyTasks` 仅返回 `assigneeId=当前用户` 且 `DONE/CANCELLED` 的归档任务（最多 200 条），每条含 `completedAt/cancelledAt/closedAt`（`closedAt = completedAt ?? cancelledAt ?? updatedAt`），按 `closedAt desc` 排序；前端可按 `closedAt` 落在 `lastWeekStart~lastWeekEnd`（上周）、`weekStart~weekEnd`（本周）或全量（全部）做切片，供后续 AI 周报汇总。`createdTasks` 返回当前用户创建且指派给他人的**待跟进**任务（非 `DONE/CANCELLED`，最多 50 条）；`collaborationTasks` 返回非 CANCELLED、非指派给当前用户、且满足「我创建 / 我评论 / 被 @」的协同任务（按 taskId 去重）；任务详情对评论参与者与被 @ 用户开放查看。**管理员工任务（admin）**：`GET /api/employee-tasks/admin-dashboard` 返回周内统计卡；`GET /api/employee-tasks/admin-users-summary` 返回按 `assigneeId` 聚合的员工汇总；`GET /api/employee-tasks/admin-tasks` 分页返回任务明细；`GET /api/employee-tasks/admin-weekly-summaries` 批量读取所有 ACTIVE 员工的 `EmployeeWeeklyAiSummary` 缓存（无缓存返回 `status=NONE` 占位，**只读 DB 不调 LLM**）；`POST /api/employee-tasks/admin-weekly-summaries/generate` 异步提交批量 AI 生成（HTTP 快速返回 `accepted/queuedCount`，后台复用 `generateWeeklyAiSummary` + `sourceHash` + single-flight + `WEEKLY_AI_CONCURRENCY`）。admin 任务数据接口（dashboard/users-summary/tasks）需 `ACTION_DASHBOARD_COMPANY_TASK_MANAGE`；`admin-weekly-summaries` GET 需 `MENU_DASHBOARD_COMPANY_MANAGEMENT`；`admin-weekly-summaries/generate` POST 需 `ACTION_DASHBOARD_COMPANY_WEEKLY_AI_GENERATE`。以上均走 `hasStrictPermission`（严格超级管理员自动通过，精确 permissionCode 通过，禁止 `*`/`ALL`/`ADMIN_FULL`/宽泛 admin 绕过）。admin 查询必须带 `weekStart`；admin 路由注册在 `/:id` 之前。

**仪表盘权限码（init-permissions）**：一级 `MENU_DASHBOARD`；子菜单 `MENU_DASHBOARD_DAILY`（每日登记）、`MENU_DASHBOARD_TASK_CENTER`（任务中心）、`MENU_DASHBOARD_COMPANY_MANAGEMENT`（公司管理）；操作 `ACTION_DASHBOARD_REMINDER_TEMPLATE_MANAGE`（挂任务中心）、`ACTION_DASHBOARD_COMPANY_TASK_MANAGE` / `ACTION_DASHBOARD_COMPANY_WEEKLY_AI_GENERATE`（挂公司管理）。运营主管、仓库主管默认授予上述 7 个码，不含用户/角色/分配账号/系统设置权限。

**上周汇总**：`GET /api/employee-tasks/weekly-summary?weekStart=YYYY-MM-DD` 实时聚合当前登录用户自己的周数据，返回规则汇总 + AI 缓存（**GET 绝不实时调用 DeepSeek**）；员工任务中心**只读** GET 缓存，无 AI 缓存时返回规则汇总/`aiStatus=RULE_ONLY`。`POST /api/employee-tasks/weekly-summary/ai-generate` 仅严格超级管理员或拥有 `ACTION_DASHBOARD_COMPANY_WEEKLY_AI_GENERATE` 可调用（普通员工返回 403：`AI周报由管理员统一生成，员工不可自行生成`）；**批量生成**统一走 `POST /api/employee-tasks/admin-weekly-summaries/generate`（需 `ACTION_DASHBOARD_COMPANY_WEEKLY_AI_GENERATE`、异步、复用 sourceHash/single-flight/并发限制）。`weekStart` 不传时默认上周周一；不允许传 `userId`。规则汇总五路查询：① 当前用户 `OperationDailyReport/OperationDailyLog` 新 5 类日报；② 收到任务 `assigneeId=当前用户` 且 `dueDate/completedAt` 落周内；③ 发起任务 `creatorId=当前用户` 且 `createdAt/dueDate/completedAt` 落周内；④ 协同任务复用 `getCollaborationParticipationTaskIds` + `collaborationTasksWhere`，周内 `createdAt/dueDate/completedAt` 过滤，最多 100 条；⑤ 运营日缺失按 `WorkdayCalendar.status=WORKDAY` 计算应登记/已登记/缺失（`REST/PENDING` 不计入）。AI `sourcePayload`（v2）含 `user`/`week`/`dailyReportSummary`/`receivedTaskSummary`/`createdTaskSummary`/`collaborationTaskSummary`（含最多 5 条 `sampleTasks` 及参与角色）/`meta.promptVersion`/`meta.hasSalesData=false`；**不含销售/订单/GMV 等平台业绩数据**。Prompt v2 要求 AI 以员工第一人称复盘（overview/highlights/risks/completionAnalysis/nextWeekSuggestions 用「我」），仅 `managerNote` 可用克制主管视角；禁止第三人称与编造销售表现。AI 缓存表 `EmployeeWeeklyAiSummary` 按 `userId+weekStart` 唯一，`sourceHash` 含 promptVersion，变更后旧缓存自然失效；`READY` 且 hash 匹配时复用。`aiSummary` 固定 6 字段（overview/highlights/risks/completionAnalysis/nextWeekSuggestions/managerNote）。环境变量 `WEEKLY_AI_ENABLED` / `DEEPSEEK_API_KEY` 控制启用；`WEEKLY_AI_PROMPT_VERSION` 默认 `v2`；`WEEKLY_AI_CRON_ENABLED` 默认 false。
...
**权限与匹配口径**：所有接口必须登录。提醒模板读取（`GET /templates`、`GET /templates/:id`）需 `MENU_DASHBOARD_TASK_CENTER` 或 `ACTION_DASHBOARD_REMINDER_TEMPLATE_MANAGE`（`hasStrictPermission`）；新增/编辑/启停/删除模板需 `ACTION_DASHBOARD_REMINDER_TEMPLATE_MANAGE`。普通员工只能查看分配给自己的今日提醒并提交自己的检查记录；有任务中心权限的用户进入 today 也只看自己适用的提醒。today 仅返回 `isActive=true` 且 assignment 命中当前 `userId` 或 `roleId` 的模板，并按 `DAILY/WORKDAY/WEEKLY` 频率过滤日期；`ABNORMAL` 检查必须填写 note。
