...
const UPLOAD_DIR   = path.join(__dirname, 'data_uploads');
const ARCHIVE_DIR  = path.join(UPLOAD_DIR, 'processed');   // 归档目录
...
function resolveCategory(row: RawRow): string | null {
...
function buildBrandList(allRows: RawRow[]): string[] {
...
function inferBrandFromTitle(title: string, allRows: RawRow[]): string | null {
  const brands = buildBrandList(allRows);
  const tLow = title.toLowerCase();
  for (const b of brands) {
    if (tLow.includes(b.toLowerCase())) return b;
  }
  return null;
}
...
interface CatInfer { l1: string; l2: string; l3: string; l4: string }

const PHONE_L1 = 'Laptop, Tablete & Telefoane';
const PHONE_L2 = 'Telefoane mobile & accesorii';

const TITLE_CAT_RULES: { test: (t: string) => boolean; cat: Omit<CatInfer, 'l1' | 'l2'> }[] = [
...
function inferCategories(title: string, brand: string | null): CatInfer | null {
...
function parseRow(row: RawRow, idx: number, fileName: string, allRows: RawRow[]): ParsedProduct | null {
...
interface FileResult {
  file:     string;
  total:    number;
  upserted: number;   // 新建 + 更新合计
  created:  number;
  updated:  number;
  skipped:  number;   // 字段缺失跳过
}

async function processFile(filePath: string): Promise<FileResult> {
  const fileName = path.basename(filePath);
  const result: FileResult = { file: fileName, total: 0, upserted: 0, created: 0, updated: 0, skipped: 0 };
  ...
  for (let start = 0; start < parsed.length; start += BATCH_SIZE) {
    ...
    for (const outcome of outcomes) {
      if (outcome.status === 'fulfilled') {
        ...
        if (outcome.value.isNew) result.created++;
        else result.updated++;
      } else {
...
function archiveFile(filePath: string) {
  if (DRY_RUN) return;   // 预检模式不归档
  const dest = path.join(ARCHIVE_DIR, path.basename(filePath));
  try {
    fs.renameSync(filePath, dest);
    console.log(`└─ ✅ 已归档至 processed/${path.basename(filePath)}`);
  } catch (e) {
...
async function main() {
  ...
  const rootFiles = (fs.readdirSync(UPLOAD_DIR, { withFileTypes: true }) || [])
    .filter((e) => e.isFile() && e.name.toLowerCase().endsWith('.json'))
    .map((e) => path.join(UPLOAD_DIR, e.name));
  const rawDir = path.join(UPLOAD_DIR, 'public_sea_raw');
  const rawFiles = fs.existsSync(rawDir)
    ? fs.readdirSync(rawDir).filter((f) => f.toLowerCase().endsWith('.json')).map((f) => path.join(rawDir, f))
    : [];
  const procFiles = fs.existsSync(ARCHIVE_DIR)
    ? fs.readdirSync(ARCHIVE_DIR).filter((f) => f.toLowerCase().endsWith('.json')).map((f) => path.join(ARCHIVE_DIR, f))
    : [];
  ...
  if (!DRY_RUN) {
    ...
    if (totSkipped > 0) console.log(`  ⚠️  跳过    : ${totSkipped} 条（字段缺失）`);
  }
  console.log('\n  各文件明细：');
  results.forEach((r) => {
